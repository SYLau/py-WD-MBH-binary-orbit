...
def create_mode(w,olap,ell,m):
    """  Create lists of modes in the form of multiple arrays """
    nn,nl,nm = len(w), len(ell), len(m)
    if len(olap) != nn:
        exit("Length of w differs from olap")

    mo_w, mo_olap, mo_ell, mo_m, mo_Wlm \
        = np.zeros(nn*nl*nm),np.zeros(nn*nl*nm),np.zeros(nn*nl*nm),np.zeros(nn*nl*nm),np.zeros(nn*nl*nm)
    ...
    for i1 in range(nn):
        for i2 in range(nl):
            for i3 in range(nm):
                ...
                ind += 1
    return mo_w, mo_olap, mo_ell, mo_m, mo_Wlm
...
def iterate(par: list[float]\
    ...
    for i in range(ntot-1):
        ...
        q1n[i+1,:] =  (q1n[i,:]+dq1[:]*np.exp(-1j*mo1_m[:]*gan[i]))*np.exp(-1j*mo1_phase)
        ...
        q2n[i+1,:] =  (q2n[i,:]+dq2[:]*np.exp(-1j*mo2_m[:]*gan[i]))*np.exp(-1j*mo2_phase)
        # mo1_old = mo1_phase
        # mo2_old = mo2_phase
...
def iterate2(par: list[float]\
            , mo1_w: list[float], mo1_olap: list[float], mo1_ell: list[float], mo1_m: list[int], mo1_Wlm: list[int], q10: list[complex]\
            , mo2_w: list[float], mo2_olap: list[float], mo2_ell: list[float], mo2_m: list[int], mo2_Wlm: list[int], q20: list[complex]\
            , ntot\
            , dq1_eq: list[complex] = None, dq2_eq: list[complex] = None\
            , flag_dq_cal = False\
            , flag_tide_reaction = True, flag_1PN = False, flag_2_5PN = False, flag_redshift = False\
...
class itmap:
    ...
    def get_mode_1(self,w,olap,ell,m, q0 = None):
        w, olap, ell, m, Wlm = create_mode(w,olap,ell,m)
        ...
        self.mo1['w'],self.mo1['olap'],self.mo1['ell'],self.mo1['m'],self.mo1['Wlm'] \
        = w, olap, ell, m, Wlm
    ...
    def get_mode_2(self,w,olap,ell,m, q0 = None):
        w, olap, ell, m, Wlm = create_mode(w,olap,ell,m)
        ...
        self.mo2['w'],self.mo2['olap'],self.mo2['ell'],self.mo2['m'],self.mo2['Wlm'] \
        = w, olap, ell, m, Wlm
    ...
    def map(self,ntot):
        ...
        an, en, gan, q1n, q2n \
        = iterate(par_vec, mo1_w, mo1_m, q10, dq1, mo2_w, mo2_m, q20, dq2, ntot\
                ,dq1_eq=None,dq2_eq=None \
                ,flag_tide_reaction = self.flag_tide_reaction \
                ,flag_1PN= self.flag_1pn\
                ,flag_2_5PN= self.flag_2_5pn\
    ...
    def map2(self,ntot):
        ...
        mo1_w, mo1_olap, mo1_ell, mo1_m, mo1_Wlm, q10 \
            = self.mo1['w'], self.mo1['olap'], self.mo1['ell'], self.mo1['m'], self.mo1['Wlm'], self.mo1['q0']
        mo2_w, mo2_olap, mo2_ell, mo2_m, mo2_Wlm, q20 \
            = self.mo2['w'], self.mo2['olap'], self.mo2['ell'], self.mo2['m'], self.mo2['Wlm'], self.mo2['q0']

        an, en, gan, q1n, q2n \
        = iterate2(par_vec, mo1_w, mo1_olap, mo1_ell, mo1_m, mo1_Wlm, q10\
                ,mo2_w, mo2_olap, mo2_ell, mo2_m, mo2_Wlm, q20\
                ,ntot\
                ,dq1_eq=None,dq2_eq=None \
...
def tutorial():
    ...
    mo1_w,mo1_olap,mo1_ell,mo1_m, mo1_Wlm = create_mode(w=[0.2],olap=[0.5],ell=[2],m=[0])
    mo2_w,mo2_olap,mo2_ell,mo2_m, mo2_Wlm = create_mode(w=[],olap=[],ell=[],m=[])
    ...
    an, en, gan, q1n, q2n \
        = iterate(par, mo1_w, mo1_m, q10, dq1, mo2_w, mo2_m, q20, dq2, ntot\
...
def tutorial_class():
    ...
    itsol.par = {'m1':m1, 'r1':r1, 'm2':m2, 'r2':r2\
                ,'a0':a0, 'e0':e0, 'ga0':ga0}
    itsol.get_mode_1(w=[0.2],olap=[0.5],ell=[2],m=[0])
    itsol.get_mode_2(w=[],olap=[],ell=[],m=[])
